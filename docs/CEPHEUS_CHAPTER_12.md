...
| Population | Cepheus population is an exponent digit and feeds map data. | Population is generated from habitability plus random spread; TS model can project actual population to exponent. | Partial | `project_uwp`; `src/domain/mainWorld/model.ts` | Add persistent actual-population state and derive PBG population multiplier. |
...
| Planetoid belts | Subsector data marks belt presence. | Rust carries a belt-present flag from generated system belts; export compresses it into PBG. | Partial | `domain/subsector.rs`; `src/domain/subsector/export.ts`; export PBG test | Track belt count, not just presence, for true PBG. |
| Gas giants | Subsector data marks gas giant presence. | Rust carries a gas-giant-present flag from generated gas/ice giants; export compresses it into PBG. | Partial | `domain/subsector.rs`; `src/domain/subsector/export.ts`; export PBG test | Track gas-giant count, not just presence, for true PBG. |
| PBG | PBG packs population multiplier, belts, and gas giants. | Text export emits PBG, but population multiplier defaults to 5 and belts/giants are binary. | Partial | `src/domain/subsector/export.ts`; `packs PBG with belt + gas-giant flags` test | Backlog task 3 should derive real PBG from actual population and system counts. |
...
2. Replace binary belt/gas-giant PBG fields with real counts from the generated
   `SolarSystem`.
3. Add a real population multiplier derived from actual population, then use it
   in PBG export.
4. Split jump routes into route categories: navigational jump adjacency,
   communication routes, and trade routes.
5. Add referee override data for bases, zones, allegiance, and routes before
